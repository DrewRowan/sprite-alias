...
(function (document, window) {
	...
	var classUtils = {
			 ...
			 * singleton
			 * 
			 * called in the same way as .create
			 * 
			 *	- creates a scoped function contructor with .create
			 *	- creates a single object with new with no arguments
			 * 
			 */
			singleton: function(){
				var classFunction,
				...
				// watch out for when a global object has been requested
				if (numArgs===2 && this.isString(args[0]) && this.isObject(args[1]) ){
					classProperties = args[1];
				}else{
					classProperties = args[0];
				}
				
				// create scroped class
				classFunction = this.create(classProperties);
				
				// create and return object (basically a singleton since the contructor function is never global)
	...
	if ( typeof define === "function" && define.amd) {
		...
		window.GE = window.GE || {};
		window.GE.classUtils = classUtils;
	}

	// auto execute the function and pass in any required globals.
	// This pattern is nice because it makes it clear what scope we expect
	// the above code to be referencing.
}(document, window));
